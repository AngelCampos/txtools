...
"GenomicRanges" package, while that of Step 2, and final 
output of the main txtools processing pipeline, is of class `data.table` from 
the "data.table" package, which is an extension of the `data.frame` class.
...
*For this example we will use the data of the "pasillaBamSubset"
...
`tx_plot_ggseqlogo()`: This function allows to quickly plot a "ggseqlogo"`
...
"Biostrings" package, or (when providing a file name) a
FASTA file.
...
- Likelihood Ratio test (from "edgeR" package): For 
