...
1. txtools introduction
2. Load read-alignments data, and the genomic and transcriptomic reference 
files into R.
3. Assign alignments to genes.
4. Summarize mapped-reads into different kinds of tables.
5. Use tx_add_* functions to add useful data and calculated metrics to the 
summarized tables.
6. Plot regions and analyses results using the tx_plot_* functions family.
7. Use tx_get_*() functions to extract non-standard information structures
from txtools tables.
8. Ise tx_test_*() functions to apply statiscal testing to detect 
...
data in a quick and simple manner. To accomplish this we propose the processing 
