...
Transcript-wise data analysis is increasingly needed to process and analyze 
RNA-seq data in which transcript-structure and nucleotide-level inspection is 
required, e.g. analyzing RNA-seq data derived from RNA-modifications detection
protocols.

**txtools** is a package that facilitates the loading of read alignmnents, from
BAM files, and processes their data into transcriptomic space, enabling their
analysis through different transcript-wise functions.
...
package, as currently it does not support transcript-wise conversion or features.


## Installation

You can install the development version from [GitHub](https://github.com/AngelCampos/txtools) with:
...
reads <- tx_load_bam(bamFile, loadSeq = T, verbose = T, yieldSize = 100000)
...
```{r, barplot_nucfreq}
tx_plot_nucFreq(resTab3[["uc010nap.1"]])
iGene <- "uc010nap.1"
...
fragments. Currently, txtools is designed for such libraries, future improvements 
will enable processing of RNA-seq libraries which are not strand-aware nor paired-end.
...
bam <- tx_load_bam(bamfiles[1], loadSeq = T)
...
txReads <- tx_reads_mc(bam, chr14_GA, withSeq = T, nCores = 10)

topGenes <- txReads %>% sapply(length) %>% sort %>% tail(10) %>% names()
covNucFreq_DT <- tx_covNucFreqDT(txReads[topGenes], geneAnnot = chr14_GA) %>% 
    lapply(function(DT) tx_add_refSeqDT(DT, fastaGenome = hg19_genome, geneAnnot = chr14_GA))
DT <- covNucFreq_DT$uc010ttb.2
tx_plot_nucFreq(covNucFreq_DT$uc010ttb.2, window_around(300, 10))
tx_plot_nucFreq(covNucFreq_DT$uc010ttb.2)
